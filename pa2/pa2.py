...
def primes(a, b):
  ...
  # we only need to test factors up to b (we're sieving up to b)
  stop = b + 1

  # initialize set P with all integers in the range [a, b]
  P = set(range(a, b + 1))

  for x in range(2, stop):

    # create set of multiples of x in [2, b]
...
def bezout_coeffs(a, b):
  ...
  # s0 and t0 hold the Bezout coefficients for (a, b)
  return {a: s0, b: t0}
...
def gcd(a, b):
  ...
  # gcd is |a * s + b * t| where s and t are Bezout coefficients
  return abs(A * bez[A] + B * bez[B])
...
def mod_inv(a, m):
  ...
  bez = bezout_coeffs(A, m)
  # bez[A] is the coefficient such that A*bez[A] + m*bez[m] = gcd(A,m)=1
  inverse = bez[A]
...
def solve_mod_equiv(a, b, m, low, high):
  ...
  # If x = m*k + a_inv * b, we need low <= x <= high
  base = (a_inv * b) % m
  # solve for k bounds: low <= m*k + base <= high
  # => (low - base)/m <= k <= (high - base)/m
  from math import ceil, floor
  k_low = ceil((low - base) / m)
  k_high = floor((high - base) / m)

  x = [m * k + base for k in range(k_low, k_high + 1)] if k_low <= k_high else []
  return set(x)
